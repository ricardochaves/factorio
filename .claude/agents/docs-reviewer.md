...
1. **Every factual claim in the changed docs against reality.**
   ...
   - Code: `.github/workflows/*.yml`, `site/build.py`, `scripts/catalog/validate.py`, `scripts/README.md` and `.gitignore`. Where `.claude/CLAUDE.md`, an agent file and the command state the same rule, they must agree: for each rule the change touches, search the other files for it and compare the sentences.
