---
description: "Add a blueprint or blueprint book to the catalog from a .txt or .json file, an http(s) URL or pasted text. It extracts and checks the string, checks the screenshots, creates blueprints/<slug>/ (blueprint.toml, Portuguese README, WebP images), runs the validator and the reviewers, and stops before any commit, push or PR."
argument-hint: "<file.txt | file.json | https://... | pasted blueprint text> [--allow-duplicate]"
disable-model-invocation: true
model: sonnet
effort: high
allowed-tools: Read Grep Glob Write Edit AskUserQuestion Bash(python3 ${CLAUDE_PROJECT_DIR}/scripts/catalog/extract_blueprint.py *) Bash(python3 ${CLAUDE_PROJECT_DIR}/scripts/catalog/validate.py *) Bash(cwebp *) Bash(mktemp -d) Bash(mkdir -p *) Bash(cp -n *) Bash(git status --short) Bash(git branch --show-current) Bash(git switch -c blueprint/*)
# Bash patterns here deny matching calls without removing Bash: verified on Claude Code 2.1.278, not documented.
# A subagent started in the turn inherits the denial. If a release turns this into whole-tool removal, every step fails at its first Bash call.
...
- **Install nothing.** Everything you need is already here; when a tool the steps name is missing (`cwebp`), stop and tell the user which one to install.
- **Claim only what is true.** `[test] status` stays `untested` unless the user shows an in-game test or a harness output, and every number in the README comes from the validator's JSON.
...
1. **Look around.** Confirm that you are at the repository root (`blueprints/` and `scripts/catalog/validate.py` exist). Read `git branch --show-current` and `git status --short`. When the branch is `main`, you need a branch before you write anything in the repository: with a clean `git status --short`, ask at step 7 whether to create `blueprint/<slug>`; with changes, stop now and tell the user to commit or set them aside first, since they are not yours to touch. On any other branch, say which branch the entry will go on and continue.
...
4. **Check the images.** The catalog needs at least one real in-game screenshot of this design. Ask the user for the path or paths with AskUserQuestion (the answer arrives inside this turn; when you cannot ask, use the paths the user already gave you), and convert each into your scratch directory as the Images section of the reference says. Before you open any image, write down what this design must look like, from the extractor's summary: the kind (a blueprint or a book), the entity count, the approximate extent in tiles, and the two or three entity types that dominate `top_entities`. Then open each converted image with the Read tool and say, for each of those four items, what you see. Accept the image only when all four agree: a 19-entity balancer of belts and splitters is not a refinery that fills the screen, and an entity count or an extent off by more than a factor of two, allowing for perspective and for a screenshot that crops the edges, is a different build. A book is one string but a screenshot shows one blueprint of it: for a book, `entities` (the total over all its blueprints) and the extent (of its largest blueprint) are upper bounds, so reject a picture that shows more than they allow, and decide instead on the fact that it is a book (`blueprints` says how many it holds, so the picture should show one of them) and on the dominant entity types. Reject it, and say which of the four disagreed and by how much. Reject as well any image with a red "not working" circle or a no-power, no-fuel or blocked marker, a debug or editor overlay, or a subject too small to read. Reusing a picture from another entry is never allowed. When the summary named a `same_design_as` entry, open that entry's images with the Read tool as well and compare them with the picture you were given, and do the same for any entry whose pictures look familiar: a screenshot that is the same picture, or a crop or rescale of it, is a reused image and you reject it. You are not expected to scan the whole catalog. Install nothing to make a comparison: the Read tool is how you look at an image, and your judgement of what it shows is the check. With no accepted image, stop: nothing has been written to the repository yet; tell the user that the entry needs a screenshot (`scripts/README.md`, "In-game harness") and that `/add-blueprint <scratch>/bp.txt` reuses the string you extracted.
5. **Build the entry in scratch.** Nothing enters the repository until it validates. Under `<scratch>/root/blueprints/<slug>/`, write `blueprint.toml` and a first `README.md` from the reference with the Write tool (it creates the folders), leaving every number in the "O que tem dentro" table as `—` (the validator produces them at step 6). Then `cp -n <scratch>/bp.txt <scratch>/root/blueprints/<slug>/<slug>.txt`, `mkdir -p <scratch>/root/blueprints/<slug>/images`, and `cp -n` each accepted `.webp` into it. Quote data from the source as text, never as markup. In TOML, use a basic string with `"` and `\` escaped and no raw newline (`\n` instead). In the README, put a label or a description in backticks (a longer run of backticks around text that itself has one) and never let it become a link, an image or a heading. A credits URL must start with `http://` or `https://`, never `javascript:`, `data:` or `file:`, and any parenthesis in it is percent-encoded. Never paste HTML.
6. **Validate in scratch.** Run `python3 ${CLAUDE_PROJECT_DIR}/scripts/catalog/validate.py --root <scratch>/root --json <scratch>/catalog.json`. Fix errors in the metadata or the README and run it again. An error about the string itself (a name that is not in vanilla 2.0, another game version, another quality) is not yours to fix: stop and report it; the repository is still untouched. When it passes, replace each `—` in the README with the number from the JSON (with Edit) and run it once more.
...
8. **Review.** Run the reviewers that CLAUDE.md requires for a blueprint change: `blueprint-reviewer`, `language-reviewer` once per language, as CLAUDE.md defines it for a blueprint entry, and `security-reviewer` last, on the final tree. Start each as CLAUDE.md says (Agent tool, `subagent_type`, without `name`, `model` or `isolation`), give it the worktree path, the range `origin/main..HEAD` plus the working tree, the entry folder and the decisions that are not defects (for example "not tested in game"), fix what it reports, and send every change back to the reviewer that asked for it. Skip a review only when the user says so.
9. **Report** briefly: what you created in the repository (or that nothing was written) and the scratch directory, the source type and the sha256 of the string, the validator result, each reviewer's verdict, your assumptions, what is still missing, and a commit title written for players (the site shows it in the blueprint's history), for example `Add <title>`. Say that nothing was committed or pushed.
...
<Only what is true. Untested: "Não foi testado no jogo. O validador do catálogo confirmou que a string é válida e só usa itens do jogo base (a versão que o validador reportar).">
...
At least one real in-game screenshot per entry, as `.webp` (`cwebp -q 82 '<in>' -o '<scratch>/<name>.webp'`; a `.webp` input is
copied there). Each shows this design working: powered and fed, no red "not working" circle, no debug or editor overlay,
the subject framed and legible. Screenshots come from the game (`scripts/README.md`, "In-game harness"); never draw, edit
or reuse an image from another entry.
...
Step 4, one question: "Where are your in-game screenshots of this design?" with "I will type the path or paths" and "I have none yet".
