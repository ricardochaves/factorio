...
- **Write to the repository in two ways.** The entry arrives as new files at step 8, and the only existing file you overwrite is the root `README.md`, and only through `python3 ${CLAUDE_PROJECT_DIR}/scripts/catalog/validate.py --readme`, which rewrites the catalog table between its markers. After step 8, when a reviewer's report asks for a fix in the entry's own files, you edit them: an Edit of a repository path is outside the grants above, so in default mode it asks the user.
