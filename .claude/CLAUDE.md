...
| `language-reviewer` | visible text changes: one instance per language (pt-BR, en-US, es), in parallel; for a new blueprint entry, pt-BR always (put the entry's README and metadata text in its scope, because the reviewer skips READMEs otherwise) and en-US or es only when the entry has English or Spanish text (`[en]`, `[es]`, `name_en`, `alt_es`, ...) | pushing |
