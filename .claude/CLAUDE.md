...
- Follow every finding of every reviewer, MINOR and NIT findings included: the reviewers are not here to play around, so take each finding seriously. A finding from `security-reviewer` is never out of scope. Dispute a finding only with evidence, to the same reviewer, and reach a conclusion together; when the reviewer keeps the finding, follow it
...
| `language-reviewer` | visible text changes: one instance per language (pt-BR, en-US, es), in parallel; for a new blueprint entry, pt-BR always (put the entry's README and metadata text in its scope, because the reviewer skips READMEs otherwise) and en-US or es only when the entry has English or Spanish text (`[en]`, `[es]`, `name_<lang>` and `alt_<lang>`) | pushing |
...
- Brief each reviewer narrowly, because it reads whatever the brief leaves open: give the worktree path, the commit range (`origin/main..HEAD` for the commits and `origin/main...HEAD` for diffs, so that a newer `origin/main` does not show up as deletions; uncommitted work counts), the files or areas of its lane that changed, the built site (`build/site`) when it matters, and the deliberate decisions that are not defects. Each reviewer's file states its lane (what it reviews and what the other reviewers own) and a budget of tool calls, because a reviewer's context grows with every call and a long review is slow and costly. Reviewers cannot ask questions, and given an empty range and a clean tree they answer `REQUEST CHANGES`.
...
- Every change made after a review, even a small one, goes back to the reviewer that asked for it, with the earlier findings listed (ID, path, one line each) and the commit of its last verdict. Resume it with SendMessage when it has run for one or two rounds; a resumed reviewer carries its whole transcript, so after that start a fresh instance of the same agent with the same brief instead. Push only when the latest state has `VERDICT: APPROVE`, and run `security-reviewer` again on the final tree right before the push.
...
- `/add-blueprint <file.txt | file.json | url | pasted text>` (`.claude/commands/add-blueprint.md`) adds a new entry from a blueprint string and does the whole job itself: it validates the design in the game, photographs it (`scripts/run_blueprint_shot.sh`) without asking for a screenshot, and corrects what is wrong in it (`scripts/catalog/edit_blueprint.py`). The exception is the consumption and production in items per second that the single-product rule (a few bullets above) asks for: the command reports it as still to do, and you calculate it afterwards. Fetching a URL and creating the branch have no grant in the command's frontmatter, so in the default permission mode the person approves those two shell commands (auto mode leaves that to its classifier). It stops before any commit or push, and `--allow-duplicate` adds a second copy of a design the catalog already holds under another label, description, icons or game version (an identical string is still refused)
