# Bloco de cidade 100 × 100 só de robôs

Modelo de lote para uma cidade só de robôs no Factorio 2.0.77, jogo base (sem Space Age): um bloco de 100 × 100 tiles com
4 roboports, borda padronizada, postes de energia, iluminação e rede logística. Os blocos se repetem de 100 em 100 tiles e
o interior do lote fica livre para o que você quiser construir. Não há trilhos, estação, geração de energia nem robôs: é só
a base de robôs.

- Arquivos, duas variantes do mesmo bloco: [`partial-concrete.txt`](partial-concrete.txt) (concreto parcial: só a borda e as bases dos postes e dos roboports têm piso; o interior do lote fica com o terreno original) e [`full-concrete.txt`](full-concrete.txt) (concreto total: concreto refinado no lote inteiro). As entidades e os fios são idênticos; só o piso muda.
- No jogo os nomes são `City block 100x100 (partial concrete)` e `City block 100x100 (full concrete)`.
- Esta é sempre a versão atual. Versões anteriores ficam no histórico do git (`git log -p -- blueprints/city-block-100x100/`).

![Bloco com concreto total](images/overview-full.webp)

## O que tem dentro

| Item | Valor |
|---|---|
| Entidades (nas duas variantes) | 120 |
| Área | 100 × 100 tiles |
...
Piso de cada variante, em unidades do item que o coloca:

| Piso | Concreto parcial | Concreto total |
|---|---|---|
| Concreto refinado | 1 932 | 8 808 |
| Concreto refinado com marca de perigo | 608 | 608 |
| Calçada de pedra (tijolo de pedra) | 584 | 584 |
| Tiles no total | 3 124 | 10 000 |

O resto dos materiais, nas duas: 76 baús de armazenagem, 24 lâmpadas, 16 postes grandes e 4 roboports.
...
Teste automatizado no Factorio 2.0.77 headless (`scenarios/city-test`), nas duas variantes, com um bloco sozinho e com 4 blocos (2 × 2, 200 × 200 tiles). Cada bloco é construído longe do centro da sua célula, para provar o alinhamento à grade. A energia vem de um poste e de uma interface de energia elétrica fora do bloco, ligados a um poste da borda; nas fotos, o fio de cobre que entra pelo canto superior esquerdo é o que liga o bloco a essa fonte, que fica fora do quadro. Cada roboport recebeu 10 robôs construtores e 10 logísticos.

| Verificação | Resultado |
|---|---|
| Importação | a string importa sem erros: 120 entidades e 3 124 (concreto parcial) ou 10 000 (concreto total) tiles, com alinhamento absoluto à grade de 100 × 100 |
...
- A base do poste do canto nordeste, em (95, 5), tem 12 tiles de concreto refinado a mais que as dos outros três cantos (em x = 87 a 89 e y = 6 a 9). Essa diferença só aparece no concreto parcial e vem assim da blueprint exportada do jogo.

## Histórico

- **Publicação** — as entidades, os fios e os pisos são os exportados do jogo. Só o nome e a descrição de cada blueprint foram reescritos (erros de digitação e nomes alinhados aos do site: concreto parcial e total).
...
```
./scripts/run_city_test.sh     # sem tela: importa, constrói, mede energia, redes e robôs
./scripts/run_city_shot.sh     # com tela: tira as fotos e grava images/*.webp
```
