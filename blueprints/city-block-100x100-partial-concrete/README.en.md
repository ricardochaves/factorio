# 100 × 100 robot-only city block, partial concrete
...
- File: [`city-block-100x100-partial-concrete.txt`](city-block-100x100-partial-concrete.txt) (partial concrete: only the edge and the pole and roboport pads are paved; the interior of the lot keeps the original terrain). In-game, its name is `City block 100x100 (partial concrete)`.
- The other variant, [full concrete](../city-block-100x100-full-concrete/), is a separate blueprint: the entities and wires are identical and only the flooring changes. The two can be placed side by side (see "Boundary between neighboring blocks").
- This is always the current version. Earlier versions are in the git history (`git log -p -- blueprints/city-block-100x100-partial-concrete/`).

![Block with partial concrete](images/overview.webp)
...
Tested in-game tile by tile, with blocks in 2 × 1, 1 × 2, 2 × 2 and 3 × 3, and also with partial concrete and full concrete alternating in a checkerboard (2 × 1, 2 × 2 and 3 × 3):
...
| Check | Result |
|---|---|
| Import | the string imports without errors: 120 entities and 3,112 tiles, with absolute snapping to the 100 × 100 grid |
...
- With partial concrete, the interior of the lot stays as it was (grass and dirt in the screenshots).

## History

- **Northeast corner fixed** — the exported version had 12 extra tiles of refined concrete on the pad of the northeast corner pole, at x = 87 to 89 and y = 6 to 9. They were removed so that the four corner pads match and the seams between blocks mirror each other. Nothing else changed.
