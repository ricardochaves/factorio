...
-- headless). Each variant is built on a grass field twice, as one block and as a 2 x 2 city, plugged into a power source and
-- loaded with robots; the power source (a big pole and an energy interface) sits 20 tiles west of the block, outside every
-- frame except its copper wire, which enters the frames that show the top-left corner of the first block.
-- Test data: scripts/export_city.py writes strings.lua. Photos go to script-output/city_<variant>__<shot>.png.
...
-- Shots per surface size (1 = one block, 2 = a 2 x 2 city), in tiles:
-- {file, center x, center y, width, height, zoom (1 = 32 px per tile), daytime (0 noon, 0.5 midnight)}.
local SHOTS = {
  [1] = {
    {"overview", 50, 50, 120, 120, 0.5, 0},
    {"roboport", 26, 26, 26, 17, 2, 0},
    {"night_corner", 14, 12, 34, 23, 1.4, 0.5},
  },
  [2] = {
    {"city_2x2", 100, 100, 220, 220, 0.3, 0},
    {"junction", 100, 100, 36, 28, 1.5, 0},   -- where four blocks meet
    {"seam", 100, 35, 34, 22, 1.6, 0},        -- the street between two blocks, with the edge poles facing each other
  },
}
...
-- raise FRAMES: one photo is taken every FRAME_STEP ticks (city_<variant>__robots_at_work_1.png ...).
local JOB_TICK, FRAME_TICK, FRAME_STEP, FRAMES = 1300, 1680, 60, 1
...
local function photo(s, variant, file, x, y, w, h, zoom, daytime)
  game.take_screenshot{surface = s, position = {x, y}, resolution = {math.floor(w * 32 * zoom), math.floor(h * 32 * zoom)},
                       zoom = zoom, path = "city_" .. variant .. "__" .. file .. ".png", show_entity_info = false,
                       daytime = daytime, water_tick = 0, force_render = true, anti_alias = true, quality = 100}
end
...
script.on_init(function()
  ...
  for _, c in ipairs(CASES) do
    ...
    for cells = 1, 2 do
      ...
      storage.surfaces[#storage.surfaces + 1] = {variant = c.name, cells = cells, key = key}
    end
  end
  inv.destroy()
...
script.on_event(defines.events.on_tick, function(e)
  ...
  if rel == JOB_TICK then
    for _, e2 in ipairs(storage.surfaces) do if e2.cells == 1 then queue_job(game.surfaces[e2.key]) end end
  elseif rel >= FRAME_TICK and rel < FRAME_TICK + FRAME_STEP * FRAMES and (rel - FRAME_TICK) % FRAME_STEP == 0 then
    ...
    for _, e2 in ipairs(storage.surfaces) do
      if e2.cells == 1 then
        local s = game.surfaces[e2.key]
        photo(s, e2.variant, file, x, y, w, h, zoom, 0)
        storage.log[#storage.log + 1] = string.format("%s %s at +%d ticks: %d wooden chests built, %d ghosts left", e2.variant, file,
          rel - JOB_TICK, s.count_entities_filtered{name = "wooden-chest"}, s.count_entities_filtered{name = "entity-ghost"})
      end
    end
  end
  if rel == SHOT_TICK then
    for _, e2 in ipairs(storage.surfaces) do
      ...
      storage.log[#storage.log + 1] = string.format("%s: first roboport has %.1f of %.1f MJ", e2.key, port.energy / 1e6, port.electric_buffer_size / 1e6)
      for _, sh in ipairs(SHOTS[e2.cells]) do
        local file, x, y, w, h, zoom, daytime = table.unpack(sh)
        photo(s, e2.variant, file, x, y, w, h, zoom, daytime)
        storage.log[#storage.log + 1] = "shot " .. e2.variant .. " " .. file
      end
    end
