...
# not headless) and converts the photos to blueprints/city-block-100x100/images/*.webp.
# Photo names: script-output/city_<name>.png -> images/<name, underscores turned into hyphens>.webp
...
OUT="$HERE/../blueprints/city-block-100x100/images"
...
mkdir -p "$OUT"
for f in data/script-output/city_*.png(N); do
  name=${${f:t:r}#city_}
  cwebp -quiet -q ${WEBP_Q:-82} "$f" -o "$OUT/${name//_/-}.webp" && echo "wrote $OUT:t/${name//_/-}.webp"
done
