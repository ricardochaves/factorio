...
| File | Purpose |
|---|---|
| `validate.py` | Checks every `blueprints/*/blueprint.toml`, decodes each string, rejects non-vanilla names, non-normal quality, game versions other than 2.0 and a missing English or Spanish translation (metadata, or an entry's `README.en.md` and `README.es.md`), a translated README whose headings, tables, links, code spans or code blocks differ from `README.md`, a link to a heading that the README does not have, and text (metadata and entry READMEs) with zero-width characters, bidirectional controls, other control or format characters or fillers that draw nothing, and computes entities, size, materials and recipes. `--json build/catalog.json` writes the index for the site; `--readme` refreshes the table in the root README, with the titles escaped. Runs in CI (`.github/workflows/validate.yml`). |
...
| `install_entry.py` | The copy step of `/add-blueprint` (step 8): `install_entry.py <entry-root> <slug>` copies `<entry-root>/blueprints/<slug>/` (the entry root is `build/add-blueprint.*/root` or `root-<k>`) to `blueprints/<slug>/` and refuses anything but the files of a catalog entry (`blueprint.toml`, `README.md`, `README.en.md`, `README.es.md`, `.txt` strings, `.webp` photos in `images/`), a link, a slug that is not lower-case words joined by `-`, an entry that already exists and an entry that `validate.py` rejects. It copies into a temporary folder in `build/`, runs `validate.py` over the copy and moves it into place in one step. Standard library only. |
...
| `run_blueprint_shot.sh` | The photo step of `/add-blueprint`: `./run_blueprint_shot.sh <blueprint.txt> <out-dir> [timeout-seconds]` builds a blueprint, or the first four of a book, in the game (scenario `blueprint-shot`, see "In-game harness"), powers it (wiring the source to every pole group), photographs its whole extent and, only after a successful run, writes `<out-dir>/shot-<n>.webp`, replacing any already there; `<out-dir>` must be inside this repository's `build/`. Needs the game and `cwebp`. |
...
- **Report.** A `shots=<n> total=<m>` line (photos taken, blueprints found) and, for each blueprint, `<built> of <all> entities
  ...
  (M counts the groups that the build's poles form, and N the groups left without power, 0 unless a wire failed), with
  `; K reached through an added wire, first at (x, y) ...` when the source's reach left K groups out, or `no poles in the
  build`; when no side accepts the source, the note is `the power source could not be placed`, followed by a `FAIL:` line,
  and a group left without power is a `FAIL:` line too. A
...
- **What the photo shows.** The game builds what can stand on grass and skips the rest without leaving a ghost, so a pumpjack
  ...
  groups, which the note names. It then connects the source with a copper wire, beyond the usual reach, to the nearest pole
  of every group that it did not reach, so the whole build is powered; on the west and south sides a strip of the pole's
  shadow or its tower can enter the margin. No ingredients are fed to the
